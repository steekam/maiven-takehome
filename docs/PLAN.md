...
| “Fetch the next 20” | Use keyset/cursor pagination, not JSON:API. Return `{ items, nextCursor }`; query 21 rows and expose a cursor only when row 21 exists. | Cursor is the pagination mechanism. JSON:API is a separate response-format specification and is unnecessary here. No `COUNT(*)` or source total needed. |
