...
def test_postgres_upsert_resume_checkpoint_and_atomic_rollback():
    ...
    try:
        with repository.ingest_lock():
            ...
            with psycopg.connect(DATABASE_URL, row_factory=dict_row) as competing_connection:
                with pytest.raises(ConcurrentRunError):
                    with PostgresIngestRepository(competing_connection).ingest_lock():
                        pytest.fail("competing lock acquisition unexpectedly succeeded")
            first_run = repository.start_or_resume_run(
            ...
            first_state = repository.commit_page(
                PageCommit(
            ...
            repository.mark_failed(first_run.run_id, "injected_restart", 0)

            resumed_run = repository.start_or_resume_run(
                fingerprint=fingerprint_one,
                unique_target=2,
                initial_url="https://api.test/api/v1/documents.json?first=1",
                new_run=False,
            ...
            resumed_state = repository.commit_page(
                PageCommit(
            ...
            report = repository.report(resumed_run.run_id)
            ...
            second_run = repository.start_or_resume_run(
            ...
            second_state = repository.commit_page(
                PageCommit(
            ...
            second_report = repository.report(second_run.run_id)
            changed_link = next(
            ...
            third_run = repository.start_or_resume_run(
            ...
            third_state = repository.state(third_run.run_id)
            ...
            repository.mark_failed(third_run.run_id, "atomic_rollback_checked", 0)

            fingerprint_four = f"integration-{suffix}-four"
            fourth_run = repository.start_or_resume_run(
            ...
            target_state = repository.commit_page(
                PageCommit(
            ...
            assert len(repository.report(fourth_run.run_id).documents) == 1
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT document_number FROM documents WHERE document_number = %s",
                    (identifiers[4],),
            ...
            fifth_run = repository.start_or_resume_run(
            ...
            source_limited_state = repository.commit_page(
                PageCommit(
        ...
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as released_connection:
            with PostgresIngestRepository(released_connection).ingest_lock():
                pass
    finally:
        connection.rollback()
